...
install-symlinks: build
	@for tool in ag base64 bunzip2 bzcat bzip2 cat curl cut find grep gunzip gzip head hexdump sed sort tail tar tee uniq unxz unzip unzstd vi wc wget xz xzcat zcat zip zstd zstdcat; do \
		ln -sf $(BINARY) $$tool; \
...
.PHONY: clean
clean:
	rm -f $(BINARY) ag base64 bunzip2 bzcat bzip2 cat curl cut find grep gunzip gzip head hexdump sed sort tail tar tee uniq unxz unzip unzstd vi wc wget xz xzcat zcat zip zstd zstdcat
	rm -rf dist/
